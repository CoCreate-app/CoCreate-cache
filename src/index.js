		...
		cache.keys().then((keys) => {
			...
			for (const key of keys) {
				...
				if (
					...
					(data.pathname.endsWith("/index.html") &&
						url.pathname + "index.html" === data.pathname) ||
					(data.pathname.endsWith("/index.html") &&
						!data.pathname.endsWith("/") &&
						url.pathname + "/index.html" === data.pathname)
				) {
					if (
						!data.host ||
						data.host.includes("*") ||
						data.host.some((host) => url.origin.includes(host))
					)
...
if (cacheBtn) {
	cacheBtn.addEventListener("click", function () {
		deleteFile("dynamic-v2", "/CoCreate-modules/CoCreate-pwa/src/index.js");
	});
}

function fileChange(data) {
	if (window.localStorage.getItem("cache") === "false") return;
	if (data.array !== "files") return;
...
navigator.serviceWorker.addEventListener("message", (event) => {
	if (event.data.action === "checkCache") {
		for (let file of Object.keys(event.data.returnedFromCache)) {
			...
			let { organization, lastModified } =
				event.data.returnedFromCache[file];
